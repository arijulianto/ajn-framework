...
if(isset($_FILES['upload'])){
	...
	if($slug1=='upload'){
		...
		$qs = $qs[1] ? '?'.$qs[1] : '';
		echo "<script>self.location.href='",MODULE_URI,$qs,"';</script>";
	}
}


// delete file
if(isset($_POST['delname']) && isset($_POST['delpath'])){
...
if($slug1=='file'){
	if($_POST['action']=='rename'){
		...
		if(is_file(MEDIA_DIR.$_POST['path'].'/'.$_POST['new_name'])){
			$output = array('status'=>'danger', 'message'=>"Cannot rename file. The file '$_POST[new_name]' was exist in this folder!");
		}else{
			if(rename(MEDIA_DIR.$_POST['file_name'],MEDIA_DIR.$_POST['path'].'/'.$_POST['new_name']))
				$output = array('status'=>'success', 'message'=>"File '$fn[1]' was renamed to '$_POST[new_name]'");
			else
				$output = array('status'=>'warning', 'message'=>'Failed to rename file. Please re-check the name and try again!');
		}
	}elseif($_POST['action']=='delete'){
		$file = end(explode('/',$_POST['file_name']));
		if(is_file(MEDIA_DIR.$_POST['file_name'])){
			if(unlink(MEDIA_DIR.$_POST['file_name']))
				$output = array('status'=>'success', 'message'=>"File '$file' was deleted successfully");
			else
				$output = array('status'=>'warning', 'message'=>'File failed to delete. Please re-check the name and try again!');
		}else{
			$output = array('status'=>'danger', 'message'=>'File failed to delete. File not exist or was deleted!');
		}
	}
	if($output)
		echo '<div class="alert alert-',$output['status'],'">',$output['message'],'</div>';
}
...
<form action="<?php echo MODULE_URI ?>/upload<?php
$qs = explode('?',$_SERVER['REQUEST_URI']);
echo isset($qs[1]) ? '?'.$qs[1] : '';
...
?></select></div>
	<div class="search"><input type="search" name="q" value="<?php echo isset($_GET['q']) ? $_GET['q'] : '' ?>" placeholder="Cari File" /></div>
</form>
</div>
...
if($ext2!='html'){
if(isset($_GET['q']) && (($_GET['q']=='' || empty($_GET['q'])) || ($_GET['q'] && stristr($file,$_GET['q'])))){
	$do = 1;
}else{
	$do = 0;
}
...
<form action="<?php echo MODULE_URI ?>/file" method="post" class="overlay dialog-file-action" style="display:none">
