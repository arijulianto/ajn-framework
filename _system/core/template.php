...
if(isset($_GET['module']) && $_GET['module']!='login'){
	if(isset($_GET['ext']) && ($ext=='json' || $ext=='xml' || $ext=='xls')){
	    ...
	    if($_GET['module']==ADMIN_DIR){
			if(!isset($_SESSION['admin_uid'])){
				if($slug1=='logout'){
			    	...
			    	exit;		
		    	}elseif($slug1=='login' && isset($_GET['logout'])){
					header('location:'.SITE_URI.ADMIN_DIR.'/login.php');
					exit;
				}
			}
		}elseif($_GET['module']!=ADMIN_DIR){
			if(isset($conf['site_login']) && !$_SESSION['user_uid']){
...
}elseif(isset($_GET['module']) && $_GET['module']=='login' && $conf['site_login']){
	if(isset($conf['login_cek'])){
