...
if(isset($module['filter']) || isset($module['input'])){
echo '<form action="" class="action-bar">',"\n";
if(isset($module['filter'])){
	foreach($module['filter'] as $fld=>$arr){
		foreach($arr as $type=>$opt){
			$opt = explode(':=>', $opt);
			$nm = $opt[0];
			$opt = explode('{', $opt[1]);
			$attr = isset($opt[1]) ? rtrim($opt[1],'}') : '';
			$opt = $opt[0];
			...
			if($type=='combo'){
				...
				foreach($dt as $v){
					$ops = explode('=>', $v);
					if(isset($ops[1]))
						echo "<option value=\"$ops[0]\"",(isset($_GET[$nm])==$ops[0]?' selected':''),">$ops[1]</option>";
					else
				...
				echo "<input type=\"search\" name=\"$nm\" value=\"",(isset($_GET[$nm]) ? $_GET[$nm] : ''),"\"$attr placeholder=\"$dt[1]\" /> ";
			}elseif($type=='lookup'){
				if(isset($_GET[$nm]) && $_GET[$nm]!=''){
					$where[] = "$fld='{$_GET[$nm]}'";
				}
