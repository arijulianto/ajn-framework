...
class Database{
	...
	public function getTables(){
		if($this->type=='mssql')
			$strQuery = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_CATALOG='".$this->database_name."'";
		elseif($this->database_type=='mysql')
			$strQuery = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_SCHEMA='".$this->database_name."'";
	...
	public function getFields($tabel,$advance=0){
		if($this->type=='mssql')
			$strQuery = "select COLUMN_NAME,COLUMN_TYPE,COLUMN_KEY,EXTRA from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = '".$tabel."' AND TABLE_CATALOG='".$this->database_name."'";
		elseif($this->database_type=='mysql')
			$strQuery = "select COLUMN_NAME,COLUMN_TYPE,COLUMN_KEY,EXTRA from INFORMATION_SCHEMA.COLUMNS where table_name='".$tabel."' AND TABLE_SCHEMA='".$this->database_name."'";
		elseif($this->database_type=='pgsql')
			$strQuery = "select COLUMN_NAME,COLUMN_TYPE,COLUMN_KEY,EXTRA from information_schema.columns where table_name='".$tabel."'";
		...
		if($strQuery){
			...
			if($query){
				...
				if(is_array($data)){
					...
					foreach($data as $k=>$v){
						if($advance){
							$res[$i] = array('name'=>$v['COLUMN_NAME'], 'type'=>$v['COLUMN_TYPE']);
							if($v['COLUMN_KEY'])
								$res[$i]['primary'] = true;
							if($v['EXTRA'])
								$res[$i]['auto_increment'] = true;
						}else{
							$res[] = $v['COLUMN_NAME'];
						}
						$i++;
