...
foreach($menu['nav'] as $link=>$mnu){
    ...
    if(is_array($mnu)){
        echo "<li",($md=='account'?' class="hidden-xs"':''),"><a href=\"",ADMIN_URI,"$link\"",MODULE==$md ? ' class="current"' : '',">",($mnu['icon']?'<i class="fa fa-'.$mnu['icon'].' nav_icon"></i>':''),"$mnu[label]",($mnu['menu']?'<span class="fa arrow"></span>':''),"</a>";
        if($mnu['menu']){
